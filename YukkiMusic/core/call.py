...
class Call(PyTgCalls):
    def __init__(self):
        ...
        for i in range(1, 6):
            ...
            if session_string:
                ...
                self.clients.append((client, pytgcalls))
    ...
    async def ping(self):
        ...
        for i in range(1, len(self.clients) + 1):
            if globals()[f"STRING{i}"]:
                pings.append(await getattr(self, f"{i}").ping())
    ...
    async def start(self):
        ...
        for i in range(1, len(self.clients) + 1):
            if globals()[f"STRING{i}"]:
                await getattr(self, f"{i}").start()

    async def decorators(self):
        for i in range(1, len(self.clients) + 1):
            if globals()[f"STRING{i}"]:
                client = getattr(self, f"{i}")
                stream_services_handler = self.create_stream_services_handler(client)
                stream_end_handler = self.create_stream_end_handler(client)
                participants_change_handler = self.create_participants_change_handler(client)
                ...
                client.on_participants_change()(participants_change_handler)

    def create_stream_services_handler(self, client):
        async def stream_services_handler(_, chat_id: int):
            await self.stop_stream(client, chat_id)

        return stream_services_handler

    def create_stream_end_handler(self, client):
    ...
    def create_participants_change_handler(self, client):
        ...
        return participants_change_handler
